...
class TrajectoryRetracing(Node):
    def __init__(self):
        ...
        self.filename = None
        self.nbr_points = 5 #number of points to take into accout to send to 
    ...
    def build_poses(self, data_path):
        ...
        for (timestamp, x, y, z, qx, qy, qz, qw) in data_path:
            ...
            #Quaternion rotation of 180°
            q_original =  np.array([qx,qy,qz,qw])
            r_original = Rotation(q_original)
            r_180 = Rotation.from_euler('z', 180, degrees=True)
            
            r_result = r_180 * r_original  
            q_result = r_result.as_quat()

            pose.pose.orientation.x = q_result[0]
            pose.pose.orientation.y = q_result[1]
            pose.pose.orientation.z = q_result[2]
            pose.pose.orientation.w = q_result[3]
            poses.append(pose)
    ...
    async def send_to_nav2(self, poses):
        while not self.action_client.wait_for_server(timeout_sec=1.0):
            self.get_logger().warn("Waiting for /follow_waypoints...")

        goal = FollowWaypoints.Goal()
        poses = poses[::self.nbr_points]
        goal.poses = poses
